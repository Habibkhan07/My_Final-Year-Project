...
@shared_task(bind=True, name="core.send_fcm_notification")
def send_fcm_notification(self, user_id: int, payload_dict: dict[str, Any]) -> None:
    ...
    Transient errors (broker/FCM timeouts, 5xx) trigger exponential-backoff
    retries up to 3 attempts. Permanent token failures mark the affected
    ``FCMDevice`` row as ``is_active=False`` so we stop paying to send to a
    dead device.
    """
    # Lazy imports — keep task module importable even if firebase_admin has
    # not been installed on a dev machine that never runs the worker.
