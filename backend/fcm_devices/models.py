"""
FCMDevice — stores Firebase Cloud Messaging device tokens.

A user can own multiple devices (phone, tablet, reinstalled app), so this is
ForeignKey — not OneToOne. Stale tokens are flagged ``is_active=False`` rather
than deleted, so we can keep an audit trail and avoid churning primary keys.
"""
