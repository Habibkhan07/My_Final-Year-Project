...
def test_sync_requires_authentication():
    client = APIClient()
    response = client.get(reverse("core_events:sync"), {"since": timezone.now().isoformat()})
...
def test_sync_filters_by_since_and_user(authed_client):
    ...
    response = client.get(reverse("core_events:sync"), {"since": cutoff})
...
def test_sync_rejects_invalid_since(authed_client):
    ...
    response = client.get(reverse("core_events:sync"), {"since": "not-a-date"})
...
def test_unacknowledged_returns_only_current_user(authed_client):
    ...
    response = client.get(reverse("core_events:unacknowledged"))
    assert response.status_code == 200
    ids = [entry["id"] for entry in response.data["results"]]
