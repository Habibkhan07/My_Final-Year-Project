...
void main() {
  ...
  group('syncMissedEvents — cursor selection', () {
    test(
      'E1 — cold start (system cursor null) → repo called with ISO of now-24h',
      () async {
        ...
        verify(
          () => repo.syncMissedEvents(
            any(
              that: predicate<String>((iso) {
                final actual = DateTime.parse(iso);
                final target = DateTime.now().toUtc().subtract(
    ...
    test(
      ...
      () async {
        ...
        verify(
          () => repo.syncMissedEvents('2026-04-24T12:00:00.000Z'),
        ).called(1);
      },
    );
  });
  ...
  group('syncMissedEvents — chronological feed', () {
    test(
      'E3 — out-of-order repo response → processEvent fed in chronological order',
      () async {
        ...
        container.listen<SystemEventState>(systemEventProvider, (prev, next) {
  ...
  group('syncMissedEvents — cascade ordering', () {
    test(
      'E4 — happy path: missed → critical → pending-acks → acknowledge',
      () async {
        ...
        expect(container.read(systemEventProvider).processedEventIds.length, 3);
      },
    );
  ...
  group('syncMissedEvents — 401 callback', () {
    test(
      'E7 — repo throws EventSyncUnauthorized → onUnauthorized fires once',
      () async {
        when(
          () => repo.syncMissedEvents(any()),
    ...
    test(
      'E8 — repo throws EventSyncNetworkFailure → onUnauthorized NOT called',
      () async {
        when(
          () => repo.syncMissedEvents(any()),
        ).thenThrow(const EventSyncNetworkFailure());
    ...
    test(
      'E9 — repo throws EventSyncServerFailure → onUnauthorized NOT called',
      () async {
        when(
          () => repo.syncMissedEvents(any()),
  ...
  group('syncUnacknowledgedCritical — direct invocation', () {
    ...
    test('E14 — repo throws EventSyncUnauthorized → onUnauthorized fired; no '
    ...
    test(
      'E15 — repo throws EventSyncServerFailure → swallowed; no propagation',
      () async {
        when(
          () => repo.fetchUnacknowledgedCritical(),
  ...
  group('acknowledge — debounce', () {
    ...
    test('E17 — 5 acks within 2s → coalesced into one POST with all 5 ids', () {
      fakeAsync((async) {
        ...
        final captured = verify(
          () => repo.acknowledgeEvents(captureAny()),
        ).captured;
        expect(captured.length, 1);
        expect((captured.single as List<String>).toSet(), {
          'a',
          'b',
          'c',
          'd',
    ...
    test('E19 — each new ack within debounce window resets the timer; 10 acks '
