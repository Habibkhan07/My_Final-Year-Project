...
void main() {
  ...
  group('dedup-map prune — 24h window', () {
    test(
        'S11a — entry older than 24h relative to incoming event → pruned',
        () {
    ...
    test(
        'S11c — boundary: entry exactly at the 24h cutoff is RETAINED '
      ...
      notifier.processEvent(
        _entity(id: 'e_boundary', timestamp: base),
      );
      notifier.processEvent(
        _entity(
          id: 'e_now',
      ...
      final ids = container
          ...
          .keys
          .toSet();
      expect(ids.contains('e_boundary'), isTrue,
          reason: 'entry at exactly the cutoff instant must be retained — '
              'isBefore is strict, so equal-timestamp survives');
      expect(ids.contains('e_now'), isTrue);
    });

    test(
        'S11d — defense in depth: hard cap fires when 500 events all sit '
      ...
      expect(ids.length, 251,
          reason: 'cap fires when len ≥ 500, prunes to 250 newest, then '
  ...
  group('processEvent — flag #19 P2 recipient filter', () {
    ...
    test(
      'S17 — event recipientUserId != current auth user → rejected '
    ...
    test(
      'S18 — currentAuthUserId is null → filter is a no-op '
      ...
      () {
        ...
        expect(accepted, isTrue,
            reason: 'with no current-user id wired, the filter must not '
    ...
    test(
      'S19 — event has no recipientUserId → filter is a no-op '
  ...
  group('processEvent — flag #19 P1 expiry filter', () {
    test(
      'S20 — event with expiresAt in the future → accepted',
      () {
        final container = _buildContainer(local);
        final notifier = container.read(systemEventProvider.notifier);
    ...
    test(
      'S21 — event with expiresAt in the past → rejected '
    ...
    test(
      'S22 — boundary: expiresAt == serverNow → rejected '
    ...
    test(
      'S23 — event with no expiresAt → filter is a no-op '
    ...
    test(
      'S24 — server-time anchor: WS source updates the anchor; expiry '
      ...
      () {
        ...
        final deviceTimeBehind =
            serverTime.subtract(const Duration(hours: 1));
        ...
        expect(
          ...
          isFalse,
          reason: 'expiry filter must use the WS-anchored server-time '
    ...
    test(
      'S25 — FCM-source events do NOT update the server-time anchor '
