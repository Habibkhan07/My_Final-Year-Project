...
void main() {
  ...
  test('C2 — created channel uses HIGH importance (heads-up notifications)',
      () async {
    await ensureJobDispatchChannel();
  ...
  test('C3 — created channel name and description are user-visible strings',
      () async {
    await ensureJobDispatchChannel();
    ...
    expect(args['name'], 'Job Requests',
        reason:
            'channel name is user-visible in OS Settings → Notifications');
    expect(
      args['description'],
  ...
  test(
      'C4 — calling ensureJobDispatchChannel twice produces two platform '
      'calls (Android dedups by id; we do not pre-check in Dart)',
      () async {
    await ensureJobDispatchChannel();
    await ensureJobDispatchChannel();
    ...
    expect(createCalls, hasLength(2),
        reason:
            'no Dart-side memoization — relies on Android dedup by id');

    // Both calls use the same id — that's what makes Android's OS-level
    // dedup safe.
    expect(
      createCalls.map(
          (c) => (c.arguments as Map<dynamic, dynamic>)['id']),
  ...
  test(
      'C5 — platform exception during channel creation does not propagate '
