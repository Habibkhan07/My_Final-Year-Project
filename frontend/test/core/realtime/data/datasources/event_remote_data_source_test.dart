...
void main() {
  ...
  test(
    'R1 — token rotation: each call re-reads from secureStorage (no DS-level caching)',
    () async {
  ...
  test(
    'R2 — fetchEventsSince: GET /events/sync/?since=...&limit=... with auth header',
    () async {
      when(
        () => client.get(any(), headers: any(named: 'headers')),
      ).thenAnswer((_) async => http.Response('[]', 200));
  ...
  test(
    'R3 — fetchUnacknowledgedCritical: GET /events/unacknowledged/ with auth header',
    () async {
      when(
        () => client.get(any(), headers: any(named: 'headers')),
      ).thenAnswer((_) async => http.Response('[]', 200));
  ...
  test(
    'R4 — acknowledgeEvents: POST /events/ack/ with {event_ids:[...]} JSON body',
    () async {
      when(
        () => client.post(
          any(),
  ...
  test(
    'R5 — registerDevice: POST /devices/register/ with {device_token, device_type}',
    () async {
      when(
        () => client.post(
          any(),
  ...
  test(
    'R6 — unregisterDevice: POST /devices/unregister/ with {device_token}',
    () async {
      when(
        () => client.post(
          any(),
  ...
  test(
    'R7 — 401 with standard envelope → HttpFailure(401, "unauthorized", "Token expired")',
    () async {
  ...
  test(
    'R8 — 500 with envelope (has "code" key) → HttpFailure preserves parsed code+message',
    () async {
      when(() => client.get(any(), headers: any(named: 'headers'))).thenAnswer(
        (_) async => http.Response(
  ...
  test(
    'R10 — 500 with non-JSON body (e.g., HTML error page) → HttpFailure with code "server_error"',
    () async {
      when(() => client.get(any(), headers: any(named: 'headers'))).thenAnswer(
        (_) async => http.Response(
  ...
  test(
    'R11 — slow response (11s) → TimeoutException (locks 10s timeout contract)',
    () {
      when(() => client.get(any(), headers: any(named: 'headers'))).thenAnswer(
