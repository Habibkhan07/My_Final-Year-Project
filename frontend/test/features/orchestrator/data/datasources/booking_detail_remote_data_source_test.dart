...
void main() {
  ...
  test('Authorization header is "Token <jwt>" when secure storage has one',
      () async {
    when(() => client.get(any(), headers: any(named: 'headers')))
        .thenAnswer(
      (_) async => http.Response(jsonEncode(bookingDetailJson()), 200),
    ...
    final headers = verify(() => client.get(any(),
  ...
  test('non-2xx with envelope throws HttpFailure carrying envelope fields',
      () async {
    when(() => client.get(any(), headers: any(named: 'headers')))
  ...
  test('non-2xx with non-JSON body falls back to generic message',
      () async {
    ...
    when(() => client.get(any(), headers: any(named: 'headers')))
        .thenAnswer(
      (_) async => http.Response('<html>500 Internal Server Error</html>',
          500),
    );
  ...
  test('204 No Content is treated as a non-2xx? (200..<300 inclusive)',
      () async {
