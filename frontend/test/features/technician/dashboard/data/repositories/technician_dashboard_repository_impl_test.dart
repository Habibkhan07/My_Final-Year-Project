...
class MockRemoteDataSource extends Mock implements ITechnicianDashboardRemoteDataSource {}
class MockLocalDataSource extends Mock implements TechnicianDashboardLocalDataSource {}

class FakeTechnicianDashboardModel extends Fake implements TechnicianDashboardModel {}
...
void main() {
  ...
  group('getDashboard', () {
    test('should return remote data and cache it when remote call is successful', () async {
      when(() => mockRemoteDataSource.getDashboard()).thenAnswer((_) async => tDashboardModel);
      when(() => mockLocalDataSource.cacheDashboard(any())).thenAnswer((_) async => {});
    ...
    test('should return cached data when remote call fails with SocketException', () async {
      when(() => mockRemoteDataSource.getDashboard()).thenThrow(const SocketException('No Internet'));
      when(() => mockLocalDataSource.getCachedDashboard()).thenAnswer((_) async => tDashboardModel);
    ...
    test('should throw DashboardNetworkFailure when remote fails and cache is empty', () async {
      when(() => mockRemoteDataSource.getDashboard()).thenThrow(const SocketException('No Internet'));
      when(() => mockLocalDataSource.getCachedDashboard()).thenAnswer((_) async => null);
    ...
    test('should throw DashboardPermissionFailure when remote returns 403', () async {
      when(() => mockRemoteDataSource.getDashboard()).thenThrow(
        const HttpFailure(statusCode: 403, code: 'permission_denied', message: 'Forbidden'),
      );
    ...
    test('should throw DashboardServerFailure when remote returns other HttpFailure', () async {
      when(() => mockRemoteDataSource.getDashboard()).thenThrow(
        const HttpFailure(statusCode: 500, code: 'server_error', message: 'Server error'),
      );
