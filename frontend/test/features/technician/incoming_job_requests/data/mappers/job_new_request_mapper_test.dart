...
void main() {
  group('JobNewRequestMapper.fromSystemEvent', () {
    ...
    test(
      '§2.5 replay (null booking_type) → defaults to BookingType.laborGig',
      () {
        final entity = JobNewRequestMapper.fromSystemEvent(
  ...
  group('JobNewRequestMapper.fromSystemEvent — flag #19 stale-FCM expiry', () {
    test('now AT expiresAt → returns null (boundary; SLA is exclusive at the '
    ...
    test('now PAST expiresAt → returns null '
    ...
    test('now JUST before expiresAt → still produces an entity '
        '(fresh by a hair — the technician still has time to swipe)', () {
      final justBeforeExpiry = _baseTs.add(
    ...
    test('now defaults to DateTime.now().toUtc() when omitted '
