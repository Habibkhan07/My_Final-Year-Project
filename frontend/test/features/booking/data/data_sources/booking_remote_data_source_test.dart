...
void main() {
  ...
  group('getTechnicianProfile', () {
    ...
    final tProfileJson = {
      ...
      'promo_tag': '20% OFF',
      'skills': [
        {'name': 'AC Repair', 'icon_name': 'ac_repair'},
      ],
      'recent_reviews': [
        {'reviewer_name': 'Sara', 'rating': 5, 'text': 'Good'},
      ],
    };
    ...
    test('200 returns TechnicianProfileModel', () async {
      when(
        () => mockClient.get(any()),
      ).thenAnswer((_) async => http.Response(jsonEncode(tProfileJson), 200));
    ...
    test('500 server_error throws HttpFailure with correct code', () async {
      ...
      when(
        () => mockClient.get(any()),
      ).thenAnswer((_) async => http.Response(jsonEncode(errorJson), 500));
  ...
  group('getAvailability', () {
    ...
    test(
      '200 returns list of AvailabilitySlotModels with correct values',
      () async {
        when(
          () => mockClient.get(any()),
        ).thenAnswer((_) async => http.Response(jsonEncode(tSlotsJson), 200));
    ...
    test(
      '200 with empty array returns empty list (technician has no schedule)',
      () async {
        when(
          () => mockClient.get(any()),
    ...
    test('builds correct URL with date and optional service params', () async {
      ...
      expect(
        captured.path,
        contains('technicians/$tTechnicianId/availability'),
      );
    ...
    test(
      'non-200 throws HttpFailure with correct code and statusCode',
      () async {
        final errorJson = {
          'code': 'not_found',
          'message': 'Technician not found.',
    ...
    test(
      '500 with non-JSON body throws HttpFailure with server_error code',
      () async {
        when(
          () => mockClient.get(any()),
  ...
  group('createInstantBooking', () {
    ...
    test(
      '201 returns InstantBookingResponseModel with correct bookingId',
      () async {
        when(
          () => mockClient.post(
            any(),
    ...
    test(
      'sends Authorization: Token header with JWT from secure storage',
      () async {
        when(
          () => mockClient.post(
            any(),
    ...
    test(
      'sends request body matching InstantBookingRequestModel.toJson()',
      () async {
        when(
          () => mockClient.post(
            any(),
    ...
    test(
      '400 out_of_service_area throws HttpFailure with correct code and message',
      () async {
        final errorJson = {
          'status': 400,
    ...
    test(
      '409 slot_unavailable throws HttpFailure with statusCode 409',
      () async {
        final errorJson = {
          'status': 409,
