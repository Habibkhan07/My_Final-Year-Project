...
void main() {
  ...
  Widget createWidgetUnderTest(DiscoveryTechnicianEntity tech) {
    return MaterialApp(
      home: Scaffold(
        body: TechnicianCard(technician: tech, onTap: () {}),
      ),
    );
  }

  group('TechnicianCard Dumb UI Widget Tests', () {
    testWidgets('should render all basic backend-provided strings correctly', (
      tester,
    ) async {
      ...
      expect(
        find.text('PROMO: Get 20% OFF the total bill for Plumbing!'),
        findsOneWidget,
      );
      ...
      expect(
        find.text('INSPECTION FEE'),
        findsOneWidget,
      ); // priceContext uppercased in bottom corner
    });

    testWidgets(
      'should fall back to primaryCategory if uiSubtitleText is null',
      (tester) async {
    ...
    testWidgets('should render promoTag badge when promoTag is provided', (
      tester,
    ) async {
    ...
    testWidgets('should not render distance if distanceKm is null', (
      tester,
    ) async {
      final noDistanceTech = tTechnician.copyWith(
        distanceKm: null,
        city: 'Lahore',
      );
