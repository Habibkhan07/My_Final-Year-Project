...
void main() {
  ...
  group('HomeNotifier', () {
    test(
      'initial build fetches home feed using default address coordinates',
      () async {
        when(
          () => mockUseCase.call(lat: 31.5204, lng: 74.3587),
    ...
    test(
      'initial build fetches without coordinates if no default address exists',
      () async {
        ...
        when(
          () => mockUseCase.call(lat: null, lng: null),
        ).thenAnswer((_) async => tHomeFeed);
    ...
    test('fetchHomeFeed explicitly updates state and coordinates', () async {
      ...
      when(
        () => mockUseCase.call(lat: 10.0, lng: 20.0),
      ).thenAnswer((_) async => tHomeFeed);
      ...
      await container
          ...
          .fetchHomeFeed(lat: 10.0, lng: 20.0);
