...
void main() {
  ...
  group('HomeRepositoryImpl Error Propagation Pipeline', () {
    ...
    test(
      'should return HomeFeedEntity when remote call is successful and cache the result',
      () async {
        // Arrange
        ...
        when(
          () => mockRemoteDataSource.getHomeFeed(
            lat: any(named: 'lat'),
            lng: any(named: 'lng'),
          ),
        ).thenAnswer((_) async => tModel);
        when(
          () => mockLocalDataSource.cacheHomeFeed(any()),
        ).thenAnswer((_) async => {});
        ...
        verify(
          () => mockRemoteDataSource.getHomeFeed(lat: tLat, lng: tLng),
        ).called(1);
    ...
    test(
      'should return cached data if SocketException occurs and cache is available',
      () async {
        const tModel = HomeFeedModel(
          categories: [],
          promotions: [],
        ...
        when(
          () => mockLocalDataSource.getCachedHomeFeed(),
        ).thenAnswer((_) async => tModel);
    ...
    test(
      'should throw HomeNetworkFailure on SocketException if no cache is available',
      () async {
        when(
          () => mockRemoteDataSource.getHomeFeed(
            lat: any(named: 'lat'),
