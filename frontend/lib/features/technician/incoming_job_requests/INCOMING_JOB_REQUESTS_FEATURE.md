...
| ~~Customer-side `job_accepted` handler~~ | ~~Cross-side obligation~~ | Wired as of flag #25 — low-urgency banner ("Booking confirmed — `<technician>` is on the way"), taps to the same `/customer/booking/:job_id` placeholder as `booking_rejected`. The doctrine-mandated per-event notifier was deferred since the customer app has no waiting-screen surface for it to drive yet; flag #26 (real `CustomerBookingDetailScreen`) is the natural place to add it. |
