...
| ~~Customer-side `job_accepted` handler~~ | ~~Cross-side obligation~~ | Wired as of flag #25 — low-urgency banner ("Booking confirmed — `<technician>` is on the way"). Taps now route to `/booking/:job_id` (the audience-shared `BookingOrchestratorScreen` from the orchestrator sprint session 3, which closed flag #26). The orchestrator screen subscribes to `systemEventProvider` via `bookingOrchestratorEventsNotifier` so the doctrine-mandated per-event refresh path is satisfied without a dedicated `JobAcceptedNotifier`. |
...
- **Sound player** (`presentation/services/incoming_job_sound_player_test.dart`): `SystemSoundIncomingJobSoundPlayer` implements the interface, `playNewOfferSound()` completes without throwing, and the right `SystemSoundType.alert` (not `click`) is dispatched on the platform channel.
- **Sheet host integration** (`presentation/widgets/incoming_job_sheet_host_test.dart`): the four `_onQueueChanged` cases — empty→arrival mounts and slides in; non-empty→empty slides out and unmounts; head change runs the vanish-reappear ceremony with sound firing exactly once after the slide-out + pause (not during slide-out, not on slide-in start); tail-only growth keeps the head visible AND does not fire the sound. Plus the `_handleAccept` 260ms hold contract — the sheet's `onAccept` callback fires the host's accept handler, which holds `removeRequest` for ~260ms so the swipe widget's confirm animation plays before the head leaves the queue.

End-to-end gesture path (a real horizontal drag on the swipe widget propagating through `DraggableScrollableSheet`'s gesture arena up to the host's accept handler) is not yet pinned — the swipe gesture is covered in isolation by the swipe widget's own test, and the host's accept hold is covered by directly invoking the sheet's `onAccept` callback. Wiring the gesture to the host through the full widget tree adds gesture-arena disambiguation noise without changing the contract anything else already pins.
