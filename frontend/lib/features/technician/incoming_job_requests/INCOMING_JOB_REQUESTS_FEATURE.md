...
| Customer-side `job_accepted` handler | Cross-side obligation | The technician-side accept flow emits `job_accepted` server-side (BOOKINGS_API.md §1.3), but the customer's Flutter app doesn't subscribe yet. See flag.md (flag #25). `booking_rejected` (decline + SLA-expiry arms) is wired as of flag #22 — low-urgency banner with reason-discriminated copy, taps to `/customer/booking/:job_id`. |
