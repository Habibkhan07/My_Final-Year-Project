...
class GoogleMapsGeocodingDataSource implements GeocodingDataSource {
  ...
  GoogleMapsGeocodingDataSource(this._client, this._apiKey)
      : assert(_apiKey.length > 0, 'GoogleMapsGeocodingDataSource requires a non-empty API key');
  ...
  Future<List<PlaceSearchEntity>> searchPlaces(
    ...
    if (status != 'OK') throw _googleErr(status, json['error_message'] as String?);

    final preds = (json['predictions'] as List? ?? const []).cast<Map<String, dynamic>>();
    return preds.map((p) {
      final struct = (p['structured_formatting'] as Map<String, dynamic>?) ?? const {};
